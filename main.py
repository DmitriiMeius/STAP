...
class TextAnalyzerApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(toolbar, text="Analyze", command=self.start_analysis).grid(row=0, column=1, padx=6)
        ttk.Button(toolbar, text="Export report", command=self.export_report).grid(row=0, column=2, padx=6)
        ttk.Button(toolbar, text="Clear", command=self.clear).grid(row=0, column=3, padx=6)

        ttk.Label(toolbar, textvariable=self.summary_var).grid(row=0, column=5, sticky="e")
        ...
        ttk.Label(right, text="Analysis").grid(row=0, column=0, sticky="w")
    ...
    def _analyze_worker(self) -> None:
        ...
        for index, path in enumerate(self.files, start=1):
            self.events.put(("status", f"Analyzing {index}/{len(self.files)}: {path.name}"))
            try:
        ...
        try:
            while True:
                ...
                if kind == "status":
                    ...
                    self.analyses = list(payload)  # type: ignore[arg-type]
                    self.render_summary()
                    self.status_var.set(f"Analysis completed: {len(self.analyses)} file(s).")
        except queue.Empty:
            pass
    ...
    def render_summary(self) -> None:
        ...
        self.output.insert(tk.END, "TOTAL\n")
        ...
        self.summary_var.set(f"Analyzed files: {len(self.analyses)}")
    ...
    def show_selected_file(self) -> None:
        ...
        self.output.insert(tk.END, "\n\nPreview\n")
        self.output.insert(tk.END, "-" * 60 + "\n")
        self.output.insert(tk.END, analysis.text[:5000])
    ...
    def export_report(self) -> None:
        ...
        default_name = f"text_analysis_{datetime.now():%Y%m%d_%H%M%S}.txt"
